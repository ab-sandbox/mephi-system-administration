...
Подключение к виртуальной машине выполнено по SSH с использованием ключевой аутентификации.

![Подключение к виртуальной машине по SSH](assets/ssh-login.png)
...
$ whoami && id
ubuntu
uid=1000(ubuntu) gid=1000(ubuntu) groups=1000(ubuntu),4(adm),20(dialout),24(cdrom),25(floppy),27(sudo),
29(audio),30(dip),44(video),46(plugdev),118(netdev),119(lxd)
```
